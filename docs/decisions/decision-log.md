...
### D-120 갈래별 소유 방식 중단 제안 철회 — 잘못된 판단과 정정 기록
- **일시**: 2026-09-18
- **단계**: Delivery Planning → Construction 경계 (functional-design 진입 시점)
- **질문 파일**: 없음 — 워크플로 진행 중 발생한 사건이며, 사용자의 되물음으로 정정되었다
- **질문**: (AI가 제안한 것) 갈래별 소유 방식이 동작하지 않으니 한 세션 진행(solo)으로 되돌릴지
- **선택지**: 한 세션에서 진행으로 바꾸기 / 멈추고 aidlc 를 먼저 고치기 / (2차 제안) 첫 단위만 진행
- **사용자 답(원문)**: "team으로 진행할 수 있는 aidlc의 버전은 몇이지?"
- **무슨 일이 있었나**:
  D-118(갈래별 소유)을 상태에 기록한 직후, 다음 단계를 꺼내오는 명령이
  `Unit Progress refresh failed: aidlc: unknown verb 'refresh-unit-progress' for engine noun 'state'`
  로 실패했다. AI 는 설치된 실행 파일 `~/.local/bin/aidlc` 의 응답만 근거로
  **"이 설치본은 갈래별 소유를 지원하지 않는다"** 고 판단하고, 사용자가 방금 확정한
  D-118 을 되돌려 한 세션 진행(solo)으로 바꾸자고 **두 차례 제안했다.** 사용자가 두 번 모두
  즉답 대신 설명을 요구했고, 세 번째로 위 질문을 되물었다.
- **확인 결과 (판단이 틀렸다)**:
  | 확인 항목 | 값 |
  |---|---|
  | 프로젝트 AI-DLC 셸 버전 (`.claude/tools/data/aidlc-stamp.json`) | 2.8.2 |
  | 설치된 실행 파일이 보고하는 버전 | 2.8.2 |
  | `refresh-unit-progress` 가 프로젝트 도구(`.claude/tools/aidlc-state.ts`)에 있는가 | 있음 (4곳) |
  | 프로젝트 도구를 직접 호출(`bun .claude/tools/aidlc-orchestrate.ts next`) | 정상 동작 |
  | 갈래별 진행표가 상태 파일에 실제로 생성되는가 | 생성됨 |
  버전이 낮아 지원되지 않는 상황이 **아니었다.** 같은 2.8.2 인데 설치된 실행 파일이
  프로젝트 셸보다 뒤처진 빌드였고, 필요한 기능은 저장소 안에 이미 있었다.
- **왜 잘못되었나**: 실행 파일 하나의 실패를 기능 부재로 단정했고, 저장소가 같이 들여온 도구나
  대체 호출 경로를 확인하지 않은 채 **사용자의 확정 결정을 되돌리자고 제안했다.** 확인 비용은
  명령 몇 줄이었고, 제안의 비용은 이미 내린 결정 하나였다.
- **결정 요약**: **D-118(갈래별 소유)과 D-119(단계마다 승인)는 그대로 유효하다.** 되돌리지 않는다.
  진행 중에는 설치된 실행 파일 대신 프로젝트 도구(`bun .claude/tools/aidlc-*.ts`)를 호출한다.
- **후속 조치**: `~/.local/bin/aidlc` 를 이 프로젝트 셸의 2.8.2 로 다시 설치한다. 팀원 5인의
  실행 파일이 같은 상태면 각자 `/aidlc` 를 쓸 때 같은 자리에서 막힌다.
- **재발 방지**: 설치된 명령이 실패해도 (1) 저장소 안 도구에 그 명령이 있는지, (2) 버전 기록,
  (3) 대체 호출 경로를 먼저 확인한 뒤에야 기능 부재로 판단한다. 확인 전에는 확정된 결정을
  되돌리자고 제안하지 않는다.
- **영향 문서**: `aidlc/spaces/default/intents/260916-scd-coaching-mvp/aidlc-state.md` (Unit Ownership 유지),
  `aidlc/spaces/default/intents/260916-scd-coaching-mvp/construction/u1-backend-foundation/functional-design/memory.md`
- **상태**: 확정 (정정 완료)
...
### D-121 · 인프라 설계(u1) 승인 — 연결 수 근거와 추적 누락을 고친 뒤

- **단계**: infrastructure-design (3.4) · **작업 단위**: u1-backend-foundation
- **결정**: 산출물 4종(`infrastructure-specification.md`, `monitoring-design.md`, `cicd-pipeline.md`,
  `traceability.json`)을 승인한다.
- **경위**: 1회차 검토는 READY 였으나 두 건이 남아 사용자가 변경 요청을 택했다.
  - **R-06 (Major)** — 본문이 인용한 NFR10.13·NFR5.2·NFR12.5 가 추적 파일의 상위 항목 목록에 없었다.
    세 항목을 상위 목록과 커버리지에 더해 25 → 28 이 되었다. R-02 와 같은 결함이 같은 단계 안에서 재발한
    것이며, 이를 계기로 **"본문이 근거로 인용한 항목은 추적에 넣는다"** 를 프로젝트 규칙으로 승격했다.
  - **R-05 (Minor)** — §2.1 의 동시 점유 상한 3 이 인용 출처(`reliability-design.md` §2)와 어긋났다.
    작업 연결이 닫힌 뒤 실패 기록 연결이 열리므로 한 작업 기준 상한은 2 이고, 3 은 두 대화의 작업이 겹칠 때
    나오는 값이다. 세 문단으로 나눠 다시 쓰고 확장식을 `1 + (작업 수 × 2)` → `1 + 작업 수` 로 정정했다.
- **바뀌지 않은 것**: 연결 풀 크기 5, 헬스체크 5초 간격 12회·한 번당 3초·시작 유예 0초, 자동 재시작 없음,
  자원 한도 없음. 근거 서술만 바뀌었고 결론값은 그대로다.
- **2회차 검토**: Critical 0 · Major 0 · Minor 0, 새 지적 없음. 판정 READY.
- **닫힌 열린 질문**: OQ-F3, OQ-N4, OQ-D1, OQ-D2 (네 건 모두 이 단계에서 닫혔다)
- **상태**: 확정

### D-122 · 걷는 뼈대(B1) 안에서도 네 사람이 단위를 나눠 맡는다

- **단계**: construction (code-generation 시작 전)
- **배경**: 초기 구상은 조원 5명 중 4명이 구현을 분할하는 것이었다. 그런데 확정된 계획에서 4갈래 병렬은
  B2~B5 부터이고, 그 앞의 B1 은 갈래 하나로 U1·U2·U3·U4 네 단위를 지난다. 그대로 두면 7주 중 상당 기간이
  1~2명으로 도는 구간이 되어 초기 구상과 어긋난다.
- **결정**: **B1 은 갈래 하나로 두되, 그 안의 네 단위를 네 사람이 각각 맡는다.**
  - 배정표의 B1 사람 역할이 이미 **"담당 1인 이상 + 리뷰 1인"** 이므로, 이는 확정 결정을 뒤집는 것이 아니라
    그 안에서 인원을 늘린 것이다.
  - 걷는 뼈대의 **"혼자 진행"** 은 *갈래가 하나*라는 뜻이며 *사람이 한 명*이라는 뜻이 아니다. 갈래 수는
    그대로 1이고 D-116(뼈대 구성)과 팀 관행의 Walking Skeleton 절은 바뀌지 않는다.

| 단위 | 담당 | 이 결정 시점의 상태 |
|---|---|---|
| U1 백엔드 기반 | 1인 | 설계 4단계 승인 완료 — **코드 생성부터 시작** |
| U2 화면 기반 | 1인 | 기능 설계부터 시작 |
| U3 녹음·전사 | 1인 | 기능 설계부터 시작 |
| U4 맥락 | 1인 | 기능 설계부터 시작 |
| — | 남는 1인 | 저장소 준비(A6), `docs/glossary.md`, 네 단위 사이 계약 대조, 통합 |

- **바뀌지 않는 것**: D-117(단위 하나를 설계부터 코드까지 끝내고 다음 단위로 간다), D-118(갈래별 소유),
  D-119(단계마다 승인), D-112(이름 배정은 Bolt 를 시작할 때 정한다), B1 승인 게이트(네 단위가 다 끝난 뒤
  한 번), 리뷰 규칙(그 단위를 만들지 않은 1인).
- **네 사람의 출발선이 다르다**: U1 은 설계가 끝나 코드로 바로 들어가고, U2·U3·U4 는 기능 설계부터 다섯
  단계를 지난다. 같은 시점에 같은 단계를 도는 것이 아니다.
- **엔진 쪽 사실**: 작업판(`team-board`)은 14개 단위 전부가 `walking-skeleton` 을 기다린다고 표시하며
  지금 가져갈 수 있는 단위가 0개다. 즉 **도구가 자동으로 갈래를 나눠 주는 지점은 B1 병합 이후**이고,
  이 결정은 그 앞 구간의 *사람* 배정을 정한 것이다. 워크플로는 여전히 단위 하나씩 지난다.
- **지금 해야 할 것**: B1 이 시작되므로 D-112 에 따라 **누가 어느 단위를 맡을지 이 시점에 정해 이 기록에
  남긴다.** 이름은 아직 비어 있다.
- **상태**: 확정

### D-123 · B1 네 단위의 담당자 확정

- **단계**: construction (B1 시작 시점) · **근거 결정**: D-112(이름 배정은 Bolt 를 시작할 때 정한다), D-122
- **결정**:

| 단위 | 크기 | 담당 | 시작 지점 |
|---|---|---|---|
| U1 백엔드 기반 | L | **양성민** | code-generation (설계 4단계 승인 완료) |
| U2 화면 기반 | M | **팀원1** | functional-design |
| U3 F01 녹음·전사 | L | **팀원2** | functional-design |
| U4 F02 맥락 구조화 | S | **팀원3** | functional-design |
| 기반·통합 (저장소 준비 A6, `docs/glossary.md`, 계약 대조) | — | **미정** | 즉시 |

- **리뷰어**: 그 단위를 만들지 않은 사람 1인. 고정하지 않는다 (팀 관행 Way of Working).
- **아직 비어 있는 자리**: 다섯 번째 사람(기반·통합)이 정해지지 않았다. 이 자리는 첫 커밋 전에 `.gitignore`
  팀 항목을 넣어야 하는 일을 들고 있어 실제로 가장 먼저 움직여야 한다.
- **상태**: 확정 (기반·통합 담당만 미정)

### D-124 · 워크플로는 한 자리에서 돌리고 구현을 나눈다

- **단계**: construction (B1 진행 방식) · **근거**: D-122, D-123 을 실제로 굴릴 수 있는 형태로 옮긴 것
- **왜 이 결정이 필요했나**: D-122 로 네 사람이 단위를 나눠 맡기로 했으나, 도구가 그 병렬을 지금 받아 주지
  않는다는 사실이 확인되었다.
  - 작업판(`team-board`)은 14개 단위 전부가 `walking-skeleton` 을 기다린다고 표시하며 가져갈 수 있는 단위가
    **0개**다. 단위를 나눠 주는 장치는 B1 이 병합된 뒤에 열린다.
  - `aidlc-state.md` 의 `Current Stage` 는 워크플로 전체에 **단일 값**이고 진행은 단위 하나씩이다. 네 사람이
    각자 워크플로를 돌리면 넷 다 같은 다음 단계로 안내받고, 상태 파일·결정 기록·감사 기록을 각자 고쳐
    **코드가 아니라 워크플로 기록에서** 충돌이 난다.
- **결정**: **AI-DLC 워크플로(설계 질문·승인·계획 생성)는 한 자리에서 돌리고, 나온 계획과 명세를 네 사람이
  각자 브랜치에서 구현한다.**
  - 워크플로를 돌리는 자리: U1 담당(양성민).
  - 네 사람의 단위 소유(D-123)는 그대로다. 소유의 범위가 *설계 단계 진행 + 구현* 에서 **구현 + 리뷰**로
    좁아진 것이다.
  - 승인 주기(D-119, 단계마다)는 그대로이며, 승인받는 자리가 한 곳으로 모인다.
  - 리뷰 규칙(그 단위를 만들지 않은 1인)은 그대로다.
- **B1 이 병합된 뒤**: 작업판이 열리므로 B2~B5 부터는 D-118 대로 갈래가 각자 단위를 집어 간다. 이 결정은
  **B1 구간에만 적용된다.**
- **열린 질문 (승인 게이트에서 확정)**: u2·u3·u4 의 설계 질문에 누가 답하는가 — 단위 담당자가 그 자리에
  함께 답하는가, 세션을 돌리는 사람이 대표로 답하는가.
- **상태**: 확정 (위 열린 질문 1건 제외)

### D-125 · u2·u3·u4 의 설계 질문은 단위 담당자가 함께 답한다

- **단계**: construction · **근거**: D-124 의 열린 질문을 닫는다
- **결정**: 워크플로는 한 자리에서 돌리되(D-124), **각 단위의 설계 질문은 그 단위 담당자가 함께 답한다.**
  담당자가 자리에 같이 있거나, 답을 미리 받아 그대로 입력한다. 세션을 돌리는 사람이 대표로 정하지 않는다.
- **근거**: 자기가 구현할 것을 자기가 정하게 되어 설계와 구현 사이의 해석 차가 줄어든다. 대표로 답하면
  구현 단계에서 "이 결정을 왜 이렇게 했는가"를 다시 설명해야 하는 자리가 생긴다.
- **적용 범위**: B1 구간(u2 화면 기반 / u3 녹음·전사 / u4 맥락). B2 부터는 갈래가 각자 워크플로를 돌리므로
  이 규칙이 필요 없어진다 (D-118).
- **상태**: 확정. 이로써 D-124 의 열린 질문이 닫혔다.

### D-126 · 저장소 ignore 규칙 보강과 첫 설계 산출물 공유

- **단계**: construction (B1 시작 준비) · **근거**: 팀 관행 `team.md` § Way of Working `[devsecops §1]`
- **배경**: `.gitignore` 는 존재했으나 내용이 전부 프레임워크 소유 블록이었고 `# END AI-DLC:gitignore` 아래가
  비어 있었다. 팀 관행이 **첫 `git add` 전에** 넣으라고 못 박은 항목이 하나도 들어 있지 않았다. 다행히 아직
  코드가 없어 비밀값이 커밋되지는 않았다(추적 중인 `.env` 없음을 확인).
- **결정**: END 마커 **아래** 별도 구역에 팀 항목을 넣는다 — `.env`, `.env.*`(단 `!.env.example`),
  `__pycache__/`, `*.py[cod]`, `.venv/`, `.pytest_cache/`, `.ruff_cache/`, `.mypy_cache/`, `.coverage`,
  `htmlcov/`, `storage/audio/`, `*.webm`. 마커 안에 넣으면 프레임워크가 블록을 다시 쓸 때 조용히 사라진다.
- **함께 올리는 것**: u1 의 설계 4단계 산출물(`construction/u1-backend-foundation/` 전체)과 D-118~D-126 결정
  기록, 갱신된 `bolt-plan.md`·`team-allocation.md`.
- **상태**: 확정
