...
- **B1 안에서도 네 사람이 단위를 나눠 맡는다** (D-122). 걷는 뼈대의 "혼자"는 *갈래가 하나*라는 뜻이지 *사람이 한 명*이라는 뜻이 아니다.
  아래 배정표의 B1 행이 이미 "담당 1인 이상"이라 적고 있으며, 그 자리를 **U1·U2·U3·U4 를 각각 맡는 네 사람**으로 채운다. 남는 1인은 저장소
  준비(A6), `docs/glossary.md`, 네 단위 사이 계약 대조와 통합을 맡는다. 초기 구상(5인 중 4인이 구현을 분할)을 뼈대 구간부터 살리기 위해서다.
  갈래 수는 그대로 1이고 B1 승인 게이트도 네 단위가 다 끝난 뒤 한 번 그대로다.
...
|---|---|---|---|---|
| B1 뼈대 | U1(범위 제한) · U2 · U3 · U4 | 혼자(한 갈래) | aidlc-developer-agent | **네 단위를 네 사람이 각각 맡는다** (D-122) — U1 / U2 / U3 / U4 담당 각 1인. 리뷰는 그 단위를 만들지 않은 1인. 남는 1인은 저장소 준비(A6), `docs/glossary.md`, 네 단위 사이 계약 대조와 통합 |
